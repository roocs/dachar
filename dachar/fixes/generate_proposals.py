...
def validate_proposal(proposal):
    ...
    if missing:
        raise KeyError(f"Required fields not provided: {missing}")

    if invalid:
        raise KeyError(f"Invalid fields provided: {invalid}")
...
def generate_fix_proposals(files):
    for file in files:
        ...
        for prop in proposal.get("fixes"):
            ...
            d = fix.to_dict()
            get_fix_prop_store().propose(d["dataset_id"]["ds_id"], d["fix"])
