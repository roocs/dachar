...
def test_arg_parse_2_ds_ids():
    ...
    parser = example_arg_parse(cmd[2:])
    assert parser.dataset_ids == 'cmip5.output1.MRI.MRI-CGCM3.rcp45.mon.ocean.Omon.r1i1p1.latest.zostoga,cmip5.output1.MOHC.HadGEM2-ES.rcp85.mon.atmos.Amon.r1i1p1.latest.tas'
    assert _to_list(parser.dataset_ids) == ['cmip5.output1.MRI.MRI-CGCM3.rcp45.mon.ocean.Omon.r1i1p1.latest.zostoga', 'cmip5.output1.MOHC.HadGEM2-ES.rcp85.mon.atmos.Amon.r1i1p1.latest.tas']
...
def test_arg_parser_paths():
    ...
    parser = example_arg_parse(cmd[2:])
    assert parser.paths == '/badc/cmip6/data/CMIP6/AerChemMIP/MOHC/UKESM1-0-LL/piClim-2xNOx/r1i1p1f2/day'
...
def test_arg_parser_facets():
    ...
    parser = example_arg_parse(cmd[2:])
    assert parser.facets == "activity=cmip5,product=output1,experiment=rcp45,frequency=mon,realm=ocean,mip_table=Omon,ensemble_member=r1i1p1,version=latest,variable=zostoga"
    assert _to_dict(parser.facets) == {'activity': 'cmip5', 'ensemble_member': 'r1i1p1', 'experiment': 'rcp45', 'frequency': 'mon', 'realm': 'ocean',
                                       'mip_table': 'Omon', 'product': 'output1', 'version': 'latest', 'variable': 'zostoga'}
