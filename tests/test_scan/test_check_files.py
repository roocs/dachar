...
def _make_nc_modify_var_attr(nc_path, var_id, attr, value):
    ...
    tmp_path = os.path.abspath(path+"tas_modify_var_attr.nc")
    return tmp_path
...
def _make_nc_modify_var_id(nc_path, old_var_id, new_var_id):
    ...
    tmp_path = os.path.abspath(path+"tas_modify_var_id.nc")
    return tmp_path
...
def _make_nc_modify_fill_value(nc_path, var_id, fill_value):
    ...
    tmp_path = os.path.abspath(path+"tas_modify_fill_value.nc")
    return tmp_path


def _make_nc_modify_coord_value(nc_path):
    ds = _open(nc_path)
    ...
    tmp_path = os.path.abspath(path + "tas_modify_coord_value.nc")
    return tmp_path


def _rename_file(nc_path):
    ds = _open(nc_path)
    # file name in this format to use with time-checker
    path = "tests/test_outputs/"
    if not os.path.exists(path):
        os.makedirs(path)
    ds.to_netcdf(path=os.path.join(path, "tas_Amon_modify_time_203012-205611.nc"))
    tmp_path = os.path.abspath(path+"tas_Amon_modify_time_203012-205611.nc")
    return tmp_path
...
def test_fail_diff_var_id():
    new_var_id = "blah"
    old_var_id = "tas"
    _f = _make_nc_modify_var_id(F1, old_var_id, new_var_id)
    file_paths = _f, F2, F3
    try:
        _check_and_open(file_paths)
    except InconsistencyError as exc:
...
def test_fail_files_missing_in_time_series():
    ...
    try:
        ...
        assert (
            str(exc)
            == "[ERROR] File not in series"
        )


def test_time_in_name_wrong():
    file_paths = F1, _rename_file(F2),  F3
    try:
        _check_and_open(file_paths)
    except InconsistencyError as exc:
        assert (
...
def test_overlaps_in_time():
    """ test failure occurs when the time series overlaps in two separate files"""
    pass



def test_fail_coord_values_different():
    file_paths = F1, _make_nc_modify_coord_value(F2), F3
    try:
        _check_and_open(file_paths)
    except InconsistencyError as exc:
        assert (
...
def test_fail_coord_attr_different():
    V = "fake"
    file_paths = F1, _make_nc_modify_var_attr(F2, "lat", 'standard_name', V), F3
    try:
        _check_and_open(file_paths)
    except InconsistencyError as exc:
        assert (
...
def test_fail_time_attr_different():
    V = "fake"
    file_paths = F1, _make_nc_modify_var_attr(F2, "time", 'standard_name', V), F3
    try:
        _check_and_open(file_paths)
    except InconsistencyError as exc:
        assert (
